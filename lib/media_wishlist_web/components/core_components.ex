defmodule MediaWishlistWeb.CoreComponents do
  @moduledoc """
  ...
  import MediaWishlistWeb.Gettext
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed hidden top-21 right-4 w-80 sm:w-96 z-50 rounded-lg p-3 shadow-md shadow-zinc-900/5 ring-1",
  ...
  def table(assigns) do
    ...
    ~H"""
    <div class="overflow-y-auto pr-2 sm:overflow-visible">
      <table class="mt-3 sm:mt-11 w-full self-center">
        <thead class="text-left text-[0.8125rem] leading-6">
          <tr>
            <th :for={col <- @col} class="p-0 pb-2 sm:pb-4 pr-6 font-bold"><%= col[:label] %></th>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(MediaWishlistWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(MediaWishlistWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc ~S"""
  Renders a table with fixed height styling.
  """
  attr(:id, :string, required: true)
  attr(:rows, :list, required: true)
  attr(:row_id, :any, default: nil, doc: "the function for generating the row id")
  ...
  def max_height_table(assigns) do
    ...
    ~H"""
    <div class="px-0 sm:overflow-visible">
  ...
  @doc """
  Renders a simple form with reduced top margin.
  """
  attr(:for, :any, required: true, doc: "the datastructure for the form")
  attr(:as, :any, default: nil, doc: "the server side parameter to collect all input under")
  ...
  def small_simple_form(assigns) do
    ~H"""
    <.form :let={f} for={@for} as={@as} {@rest}>
